...
-- Cíntia (enfermeira) - Centro
UPDATE caregiver.caregivers
SET location = ST_SetSRID(ST_MakePoint(-45.9701, -23.3041), 4326)::GEOGRAPHY
WHERE crm_coren = 'COREN-SP 123456';

-- Roberto (médico) - Jardim Santa Maria
UPDATE caregiver.caregivers
SET location = ST_SetSRID(ST_MakePoint(-45.9699, -23.3035), 4326)::GEOGRAPHY
WHERE crm_coren = 'CRM-SP 654321';

-- Aline (fisioterapeuta) - Jardim Paraíso
UPDATE caregiver.caregivers
SET location = ST_SetSRID(ST_MakePoint(-45.9688, -23.3058), 4326)::GEOGRAPHY
WHERE crm_coren = 'CREFITO 987654';

-- Lucas (técnico de enfermagem) - Jardim Santa Maria
UPDATE caregiver.caregivers
SET location = ST_SetSRID(ST_MakePoint(-45.9715, -23.3027), 4326)::GEOGRAPHY
WHERE crm_coren = 'COREN-SP 456789';

-- Juliana (psicóloga) - Jardim Flórida
UPDATE caregiver.caregivers
SET location = ST_SetSRID(ST_MakePoint(-45.9722, -23.3049), 4326)::GEOGRAPHY
WHERE crm_coren = 'CRP 112233';

UPDATE caregiver.caregivers
SET rating = CASE 
        WHEN crm_coren = 'COREN-SP 123456' THEN 4.8
        WHEN crm_coren = 'CRM-SP 654321' THEN 4.5
        WHEN crm_coren = 'CREFITO 987654' THEN 4.2
        WHEN crm_coren = 'COREN-SP 456789' THEN 3.9
        WHEN crm_coren = 'CRP 112233' THEN 4.9
    END,
    review_count = CASE 
        WHEN crm_coren = 'COREN-SP 123456' THEN 15
        WHEN crm_coren = 'CRM-SP 654321' THEN 8
        WHEN crm_coren = 'CREFITO 987654' THEN 10
        WHEN crm_coren = 'COREN-SP 456789' THEN 4
        WHEN crm_coren = 'CRP 112233' THEN 22
    END;
