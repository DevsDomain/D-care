...
const categoryNames = {
  instrumental: 'Atividades Instrumentais',
  cognitive: 'Função Cognitiva',
...
export default function IvcfAssessment() {
  ...
  const submitAssessment = async () => {
    ...
    setLoading(true);
    try {
      const response = await mockApi.submitIvcfAssessment(selectedElder.id, answers);
      if (response.success && response.data) {
      ...
      <div className="min-h-screen bg-background pb-24">
        ...
        <div className="bg-card border-b border-border px-4 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold text-foreground">Resultado IVCF-20</h1>
        ...
        <div className="p-4 space-y-6">
          ...
          <Card className="healthcare-card">
            <CardContent className="pt-6 text-center">
              ...
              >
                {result.category === 'independent' ? 'Independente' : 
                 result.category === 'at-risk' ? 'Em Risco' : 'Fragilizado'}
              </Badge>
              
              <p className="text-sm text-muted-foreground mt-2">
                Avaliação realizada em {new Date(result.completedAt).toLocaleDateString('pt-BR')}
              </p>
            </CardContent>
          </Card>
                  ...
                  <div key={category} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Icon className="w-4 h-4 text-healthcare-light" />
            ...
            <CardContent>
              <div className="space-y-3">
                {result.tips.map((tip, index) => (
                  <div key={index} className="flex items-start gap-3 p-3 bg-healthcare-soft rounded-xl">
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="bg-card border-b border-border px-4 py-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-semibold text-foreground">Avaliação IVCF-20</h1>
            <p className="text-sm text-muted-foreground">
      ...
      {currentQ && (
        <div className="p-4 space-y-6">
          {/* Category indicator */}
          <div className="flex items-center gap-2">
            {React.createElement(categoryIcons[currentQ.category], {
